...
def pull_config(task: Task):
    with open(f'inventory/intended/configs/{task.host.name}.cfg', "r") as f:
...
def deploy_config(task: Task):
    with open(f'inventory/intended/configs/{task.host.name}.cfg', "w") as f:
...
def run():
    ...
    nr = InitNornir(config_file="pyavd_config.yml")
...
# docs = input("Run Ansible playbook to generate documentation (y/n)? ")
# if docs.lower() == "y":
#     subprocess.Popen('ansible-playbook playbooks/avd_lab-fabric-deploy-cvaas.yml --tags build', shell=True)
